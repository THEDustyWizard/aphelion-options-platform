...
const schwab_service_1 = require("./schwab.service");
...
class RecommendationService {
    ...
    pythonProcess;
    constructor() {
        this.logger = new logger_1.Logger('RecommendationService');
        this.schwabService = new schwab_service_1.SchwabService();
        this.newsService = new news_service_1.NewsService();
        this.pythonProcess = null;
    }
    async generateRecommendation(request) {
        const { ticker, sector, lookbackDays = 60, includeOptionsAnalysis = true, includeNewsSentiment = true } = request;
        // Fetch market data
        let marketData = null;
        try {
        ...
        const currentPrice = marketData?.lastPrice || marketData?.mark || 100;
        // Build scores
        const scores = {
            technical: 50,
            fundamental: 50,
    ...
    async generateBatchRecommendations(requests, maxConcurrent = 5) {
        const results = [];
        for (let i = 0; i < requests.length; i += maxConcurrent) {
            const batch = requests.slice(i, i + maxConcurrent);
            const batchResults = await Promise.allSettled(batch.map(req => this.generateRecommendation(req)));
    ...
    async getRecentRecommendations(ticker, limit) {
        try {
            const repo = database_1.DatabaseConfig.getRepository(recommendation_model_1.Recommendation);
    ...
    async getTopRecommendationsBySector(sector, limit) {
        try {
            const repo = database_1.DatabaseConfig.getRepository(recommendation_model_1.Recommendation);
            return await repo.find({
    ...
    async backtestRecommendation(id, endDate) {
        try {
            const repo = database_1.DatabaseConfig.getRepository(recommendation_model_1.Recommendation);
            const recommendation = await repo.findOne({ where: { id } });
            if (!recommendation) {
                throw new Error(`Recommendation ${id} not found`);
            }
            let historicalData = null;
            try {
                const daysBack = Math.ceil((endDate.getTime() - recommendation.generatedAt.getTime()) / (1000 * 60 * 60 * 24));
                historicalData = await this.schwabService.getHistoricalPrices(recommendation.ticker?.symbol || '', {
                    periodType: 'day',
    ...
    async updateRecommendationStatus(id, updates) {
        try {
            const repo = database_1.DatabaseConfig.getRepository(recommendation_model_1.Recommendation);
            const recommendation = await repo.findOne({ where: { id } });
            if (!recommendation) {
            ...
            return await repo.save(recommendation);
        }
        catch (error) {
    ...
    selectStrategy(scores) {
        const total = scores.total || 50;
        if (total >= 70)
            return { name: 'Long Call', direction: 'bullish' };
        if (total >= 60)
            return { name: 'Bull Call Spread', direction: 'bullish' };
        if (total <= 30)
            return { name: 'Long Put', direction: 'bearish' };
        if (total <= 40)
            return { name: 'Bear Put Spread', direction: 'bearish' };
        if (total >= 45 && total <= 55)
            return { name: 'Iron Condor', direction: 'neutral' };
        return { name: 'Strangle', direction: 'neutral' };
    }
    /**
     * Calculate option parameters (expiration and strike)
     */
    calculateOptionParameters(_scores, data, strategy) {
     ...
     */
    calculateValidUntil(expirationDays) {
    ...
    async saveRecommendation(result) {
        try {
            const tickerRepo = database_1.DatabaseConfig.getRepository(ticker_model_1.Ticker);
            const recRepo = database_1.DatabaseConfig.getRepository(recommendation_model_1.Recommendation);
            ...
            await redis_1.RedisConfig.setCached(`recommendation:${result.ticker}:latest`, result, 3600 // Cache for 1 hour
            );
        }
        catch (error) {
     ...
     * Analyze options chain
     */
    analyzeOptions(optionChain) {
        if (!optionChain)
            return null;
        const calls = optionChain.callExpDateMap ? Object.values(optionChain.callExpDateMap).flatMap((exp) => Object.values(exp).flat()) : [];
        const puts = optionChain.putExpDateMap ? Object.values(optionChain.putExpDateMap).flatMap((exp) => Object.values(exp).flat()) : [];
        const allOptions = [...calls, ...puts];
        if (allOptions.length === 0)
            return null;
        // Calculate average implied volatility
        const ivs = allOptions.map(opt => opt.impliedVolatility || 0).filter(iv => iv > 0);
        ...
        const totalVolume = allOptions.reduce((sum, opt) => sum + (opt.totalVolume || 0), 0);
        const totalOpenInterest = allOptions.reduce((sum, opt) => sum + (opt.openInterest || 0), 0);
     ...
     */
    findAtTheMoneyStrike(optionChain, currentPrice) {
        if (!optionChain || !optionChain.callExpDateMap)
            return currentPrice;
        const calls = Object.values(optionChain.callExpDateMap).flatMap((exp) => Object.values(exp).flat());
        if (calls.length === 0)
            return currentPrice;
        // Find strike closest to current price
        return calls.reduce((closest, opt) => {
            if (!opt.strike)
                return closest;
        ...
        }, calls[0].strike || currentPrice);
    }
    /**
     * Find strike above price
     */
    findStrikeAbove(price, percentAbove) {
     ...
     */
    findStrikeBelow(price, percentBelow) {
     ...
     */
    simulateTrade(recommendation, historicalData) {
